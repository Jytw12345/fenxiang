...
function eventTagClass(ev) {
  if (ev === 'open') return 'tag on';
  if (ev === 'heartbeat') return 'tag blue';
  if (ev === 'progress') return 'tag';
  if (ev === 'close' || ev === 'timeout') return 'tag off';
  return 'tag';
}
function fmtProg(p) {
  if (!p) return '—';
...
window.openMemberDetail = async (memberId) => {
  ...
  const logs = (d.logs || []).length
    ? `<table class="vt log-table"><tr><th>时间</th><th>事件</th><th>文件</th><th>进度</th></tr>${d.logs.map(l => {
...
window.toggleStoreMembers = async (id) => {
  ...
  try {
    ...
    box.innerHTML = `<table class="vt"><tr><th>邮箱</th><th>姓名</th><th>角色</th><th>操作</th></tr>${ms.map(m => {
      ...
      return `
      <tr class="${isSuper ? 'muted' : ''}">
        <td>${esc(m.email)} ${isSuper ? '<span class="tag on">超管</span>' : ''}</td>
        <td>${esc(m.realName || '—')}</td>
        <td>${m.role === 'admin' ? '店长' : '员工'}</td>
        <td class="acts-cell">${ops}</td>
      </tr>`;
    }).join('')}</table>`;
...
async function loadFiles() {
  ...
  box.innerHTML = `<div class="card"><h2>文件管理</h2>
    <div class="audit-bar">
      ...
      ${isSuperNow() ? '<div class="ab grow"><label>按人员筛选（仅超管）</label><input id="fileOwner" placeholder="输入邮箱关键词，如 309953160" /></div>' : ''}
