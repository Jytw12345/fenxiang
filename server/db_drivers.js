...
const SCHEMA = {
  ...
  mysql: [
    ...
    `CREATE TABLE IF NOT EXISTS shares (id VARCHAR(64) PRIMARY KEY, file_id VARCHAR(64), owner_id VARCHAR(64), owner_token VARCHAR(64), name TEXT, kind TEXT, status VARCHAR(16) DEFAULT 'active', max_viewers INT DEFAULT 0, max_views INT DEFAULT 0, duration_sec INT DEFAULT 0, expires_at BIGINT, access_code TEXT, auth_mode VARCHAR(16) DEFAULT 'open', watermark TEXT, disable_copy INT DEFAULT 1, disable_print INT DEFAULT 1, disable_download INT DEFAULT 1, disable_screenshot INT DEFAULT 0, self_destruct INT DEFAULT 0, extra TEXT, created_at BIGINT, updated_at BIGINT)`,
...
async function createDriver() {
  ...
  // 兼容旧库：为 shares 补齐 self_destruct 列（阅后即焚 / 上限自毁开关）
  try { await drv.exec('ALTER TABLE shares ADD COLUMN self_destruct INTEGER DEFAULT 0'); } catch (e) { /* 列已存在，忽略 */ }
